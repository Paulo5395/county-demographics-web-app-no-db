...
def render_fact():
    ...
    data = county_highest_avg_income(state)
    data2 = county_most_under_18(state)
    fact = "In " + state + ", the county with the highest average income is " + data[0] + " with an average income of " + str(data[1]) + "."
    fact2 = "In " + state + ", the county with the highest number of people under 18 is " + data2 + "."
...
def county_highest_avg_income(state):
    with open('demographics.json') as demographics_data:
        counties = json.load(demographics_data)
    highest=0
    county=""
    for c in counties:
        if c["State"] == state:
            if c["Income"]["Per Capita Income"] > highest:
                highest = c["Income"]["Per Capita Income"]
                county = c["County"]
